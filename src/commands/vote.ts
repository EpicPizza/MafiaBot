    ...
    execute: async (interaction: ChatInputCommandInteraction | ContextMenuCommandInteraction | Command | AutocompleteInteraction) => {
        ...
        if(player == "NEEDS REFRESH") {
            ...
            if(!user || !voter) {
                ...
                let message = voter.nickname + (voted ? " voted for " : " removed vote for ") + user.nickname + "!" + (half - specific.length < 4 && half - specific.length > 0 ? " " + Math.ceil(half - specific.length) + " vote" + (half - specific.length == 1 ? "" : "s") + " until hammer!" : "");
                ...
                if('arguments' in interaction) {
                    ...
                    if(half - specific.length < 4 && half - specific.length > 0) {
                        await setup.primary.chat.send(Math.ceil(half - specific.length) + " vote" + (half - specific.length == 1 ? "" : "s") + " until hammer!");
                    }
                } else {
